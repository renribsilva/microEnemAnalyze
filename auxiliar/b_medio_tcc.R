...
itens_caderno <- itens_2019 |>
  ...
  filter(is.na(TP_LINGUA) | TP_LINGUA == 0 | TP_LINGUA == "")
...
itens_mirt$d <- as.numeric(itens_mirt$a1) *
...
cli::cli_alert_info(
  "B médio (escala original) extraído do mod: {.val {round(b_extraido_mod, 3)}}"
)
cli::cli_alert_success(
  "B médio (escala ENEM) extraído do mod: {.val {round(b_enem_mod, 1)}}"
)
...
# 3. Mapear colunas de acerto (P1) - Se o arquivo tem
# P0 e P1, P1 são as colunas pares
...
b_medio_ing_enem <- mean(
  itens_caderno$NU_PARAM_B[c(idx_comum, idx_ing)],
  na.rm = TRUE
) *
  k_area +
  d_area
b_medio_esp_enem <- mean(
  itens_caderno$NU_PARAM_B[c(idx_comum, idx_esp)],
  na.rm = TRUE
) *
  k_area +
  d_area
...
ggplot(df_plot, aes(x = theta_enem, y = acertos_esperados, color = lingua)) +
  ...
  geom_vline(
    xintercept = b_medio_ing_enem,
    linetype = "dashed",
    color = "#00BFC4"
  ) +
  geom_vline(
    xintercept = b_medio_esp_enem,
    linetype = "dashed",
    color = "#F8766D"
  ) +
  # Estética
...
cli::cli_alert_info(
  "Dificuldade Média ENEM (Inglês): {.val {round(b_medio_ing_enem, 1)}}"
)
cli::cli_alert_info(
  "Dificuldade Média ENEM (Espanhol): {.val {round(b_medio_esp_enem, 1)}}"
)
cli::cli_alert_success(
  "Ponto de maior discriminação (Inglês):
  {.val {round(theta_inflexao_enem, 1)}} na Escala ENEM"
)
