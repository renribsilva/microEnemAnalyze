...
path_json <- as.character(
  "~/Área\ de\ trabalho/DEV/NEXT/microenem/src/app/(home)/JSON/2019/"
)
...
write_iteminfo(path_json = path_json, ano = 2019)

#-------------------------------------------------------------
# Escreve curva característica do teste de todos os cadernos -
#-------------------------------------------------------------

score_lc <- data.table::fread("exploration/2019/MICRODADOS/score_lc.csv")
score_ch <- data.table::fread("exploration/2019/MICRODADOS/score_ch.csv")
score_cn <- data.table::fread("exploration/2019/MICRODADOS/score_cn.csv")
score_mt <- data.table::fread("exploration/2019/MICRODADOS/score_mt.csv")
