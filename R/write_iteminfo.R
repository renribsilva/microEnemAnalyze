...
#' dados em formato JSON compatível com a estrutura do ApexCharts ou Chart.js.
#'
#' @param ano Inteiro ou caractere indicando o ano para buscar
...
write_iteminfo <- function(path_json = NULL, co_prova = NULL, ano) {
  ...
  for (codigo in names(lista_modelos)) {
    tryCatch(
      ...
      error = function(e) {
        cli::cli_alert_danger(
          "Erro ao extrair dados do caderno {.val {codigo}}: {e$message}"
        )
      }
    )
  }
  ...
  if (!is.null(path_json)) {
    ...
    final_file <- if (grepl("\\.json$", path_json)) {
      ...
      file.path(path_json, paste0("iteminfo_", ano, ".json"))
    }
