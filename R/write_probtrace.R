...
#'
#' @description Esta função processa modelos MIRT para cadernos do ENEM,
#' extrai as curvas de probabilidade de acerto e exporta os dados em
#' formato JSON compatível com a estrutura do Chart.js.
#'
#' @param ano Inteiro ou caractere indicando o ano (ex: 2019) para buscar
#' os objetos itens_ano e dic_ano.
#' @param path_json Opcional. String com o caminho do diretório ou
...
write_probtrace <- function(path_json = NULL, co_prova = NULL, ano) {
  ...
  for (codigo in dic_df_p1$codigo) {
    ...
    dados_prova <- dados_prova[
      order(dados_prova$TP_LINGUA, dados_prova$CO_POSICAO),
    ]

    if (nrow(dados_prova) > 0) {
      lista_nomes_itens[[as.character(codigo)]] <- as.character(
        dados_prova$CO_ITEM
      )

      # Criar máscara para identificar itens sem parâmetros
      lista_mask_na[[as.character(codigo)]] <- is.na(dados_prova$NU_PARAM_A) |
        is.na(dados_prova$NU_PARAM_B)

      # Preparar df para o MIRT (substituindo NAs por valores dummy
      # apenas para não quebrar a geração do objeto)
      ...
      tryCatch(
        {
          lista_modelos[[as.character(
            codigo
          )]] <- mirtCAT::generate.mirt_object(df_mirt, "3PL")
          cli::cli_status_update(
            id = cp_mod,
            msg = "Modelo gerado: {codigo} ({nrow(df_mirt)} itens)"
          )
        },
        error = function(e) {
  ...
  for (codigo in names(lista_modelos)) {
    tryCatch(
      ...
      error = function(e) {
        cli::cli_alert_danger(
          "Erro ao extrair dados do caderno {.val {codigo}}"
        )
      }
    )
  }
  ...
  if (!is.null(path_json)) {
    ...
    final_file <- if (grepl("\\.json$", path_json)) {
      ...
      file.path(path_json, paste0("probtrace_", ano, ".json"))
    }
    final_path_clean <- path.expand(final_file)
    dir.create(
      dirname(final_path_clean),
      showWarnings = FALSE,
      recursive = TRUE
    )
